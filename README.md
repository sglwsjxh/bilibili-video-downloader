...
- ✅ **并行下载** – 视频轨和音频轨同时下载，实时双进度条显示  
- ✅ **智能选择** – 自动选取最高分辨率视频轨和最高码率音频轨  
- ✅ **单轨重试** – 下载失败仅重试失败轨道，已完成文件不会丢失  
...
### 4. 自定义下载路径
- 默认下载目录为 `C:\Users\用户名\Downloads`，如需修改，请打开 `popup.js`，找到 `downloadsDir` 变量（约第 66 行），将其改为你的实际下载目录（注意使用双反斜杠 `\\` 或正斜杠 `/`）
...
- 如果检测成功，面板会显示视频标题、自动选中的画质信息（如 `[1080P] 1920x1080 · 6000kbps`）和 **"⬇️ 下载视频"** 按钮。
- 如果显示"未检测到完整视频流"，请**刷新页面**（F5）或先播放视频，再次点击图标。

### 第三步：下载并合成
点击 **"⬇️ 下载视频"**，插件将并行执行：
1. **同时下载**视频流（`video.mp4`）和音频流（`audio.mp3`），两个进度条实时显示下载进度
2. 两轨下载完成后，自动调用 FFmpeg 合并两个文件，生成 `[视频标题].mp4`
3. 合并完成后自动删除临时文件
4. **单轨失败**：若其中一轨下载失败，已完成轨道文件会保留，可点击对应重试按钮重新下载失败轨道
...
1. **解析**：`content.js` 注入 B 站页面，从 `<script>` 标签中提取 DASH 流的全部候选视频/音频轨，自动选取最高分辨率视频轨和最高码率音频轨
2. **通信**：点击下载后，`popup.js` 通过端口（`chrome.tabs.connect`）与 `content.js` 建立长连接，并行下发视频和音频下载指令
3. **下载**：`content.js` 使用流式 `fetch` + `ReadableStream` 下载，实时通过端口回传已下载字节数，`popup.js` 更新双进度条
4. **合成**：两轨下载完成后，构造 `ffmpeg-run://` 协议 URL，浏览器唤起本地注册的 `run-ffmpeg.vbs` 脚本，脚本执行 FFmpeg 命令合并音视频，并清理临时文件
